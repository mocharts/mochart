...
strip** while Reset reports enabled **in the menu**: the same elements, no mirroring.
...
builder first, as a separately verified pure refactor. Any new port should do the same, since it
...
Build the controller in `firstUpdated`, never `connectedCallback`, because `@query` is a lazy
