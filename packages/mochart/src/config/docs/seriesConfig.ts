...
export default function getDescriptions() {
  return {
    ...
    title: 'the title to display for the series in the legend (use null or an empty string for none)',
    ...
    focusCategoryOnHover: 'whether the category should be focused while the user hovers the pointer over a category of the series in the chart',
    focusCategoryOnClick: 'whether a click/tap on a category of the series should pin the focus on that category, so it stays focused after the pointer leaves (a second click releases it)',
...
export function getDetails() {
  return {
    ...
    axis: 'Assigns the series to the value axis in `valueAxes` whose `id` matches. With a single configured axis this can be omitted, as it defaults to that axis id.',
    ...
    curve: 'Only affects the `line` and `area` renderers. `type` selects the d3-shape curve (`linear`, `monotoneX`, `natural`, `step`, `cardinal`, `catmullRom`, ...) and `param` sets the tension of a `cardinal` curve or the alpha of a `catmullRom` one. These are the only two types with a configurator, so the others reject it.',
