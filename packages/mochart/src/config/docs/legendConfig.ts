  ...
  return {
    ...
    truncation: {
      ...
      properties: {
        ...
        maxFraction: 'the maximum fraction (0 - 1) of the legend width to allow any one legend item to occupy'
      }
    },
...
export function getDetails() {
  return {
    truncation: { properties: { maxFraction: 'At the default `1` a single item may take the whole legend width, so a long title wraps onto a row of its own and the legend grows downward at the plot\u2019s expense. A lower fraction limits every item to that share of the legend width and truncates the titles that exceed it, so `0.5` fits two items to a row and `0.34` fits three. The limit never takes an item below the `accessibility.minTargetSize` floor.', tooltipEnabled: 'When `true`, a truncated legend item carries an svg `<title>` holding the full text, which browsers show as their native tooltip (not the chart `tooltip`) while a mouse or pen rests on it. Touch has no hover, so nothing shows there; a keyboard-reachable item is already named from the full series title.' } },
