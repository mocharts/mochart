...
describe('expansion final axis bases (raw-only domain change)', () => {
  ...
  it('moves the base with the raw domain even though the filtered domain is unchanged', () => {
    ...
    // an un-ranged bar is drawn from the axis end, so that is what it animates to
    expect(final.seriesData.seriesBases[seriesB]).toBe(final.seriesData.raw.renderAxisDomains[axisId][0]);
    // the entering bar starts at the base, so it has zero height instead of spanning the old base to the new min
    expect(getStartChartData(cad).seriesData.raw.values[seriesB].plain![2]).toBe(final.seriesData.seriesBases[seriesB]);
  });
});
