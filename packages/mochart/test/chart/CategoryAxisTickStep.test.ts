 ...
 * offset, or under a unit the first category of each calendar period — thinned to what fits, with a
...
describe('category axis tick step on an ordinal axis', () => {
  ...
  it('steps by week to the first trading day of each week, across a holiday and a partial first week', () => {
    ...
    const { container, chart } = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%b %d' }, tickStep: { unit: 'week' } }, dateRows(dates));
  ...
  it('skips the partial first week with an offset, and steps every second week with a count', () => {
    ...
    const skipped = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%b %d' }, tickStep: { unit: 'week', offset: 1 } }, dateRows(dates));
    expect(getAxisLabels(skipped.container)).toEqual(['Jun 08', 'Jun 15', 'Jun 22', 'Jun 29']);
    skipped.chart.destroy();
    const fortnightly = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%b %d' }, tickStep: { unit: 'week', count: 2 } }, dateRows(dates));
  ...
  it('steps by month and year to the first category of each period', () => {
    ...
    const monthly = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%b %d' }, tickStep: { unit: 'month' } }, dateRows(dates));
    expect(getAxisLabels(monthly.container)).toEqual(['Nov 10', 'Dec 08', 'Jan 05', 'Feb 02']);
    monthly.chart.destroy();
    const yearly = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%Y' }, tickStep: { unit: 'year' } }, dateRows(dates));
  ...
  it('thins the surviving ticks to what fits, keeping them on the rule', () => {
    ...
    const { container, chart } = renderChart({ type: 'date', scale: 'ordinal', tickLabel: { format: '%b %d' }, tickStep: { unit: 'week' } }, dateRows(dates), 300);
...
describe('category axis tick step on a linear date axis', () => {
  it('places the ticks on the period boundaries', () => {
    const dates = weekdays('2026-06-03', 19, ['2026-06-08']);
    const { container, chart } = renderChart({ type: 'date', scale: 'linear', tickLabel: { format: '%b %d' }, tickStep: { unit: 'week' } }, dateRows(dates));
...
describe('category axis tick step validation', () => {
  it('rejects the ordinal-only members on a linear axis and a unit on a non-date axis', () => {
    ...
    const stringUnit = enhanceConfig({
      version: '1.0.0',
      categoryAxis: { property: 'label', type: 'string', scale: 'ordinal', tickStep: { unit: 'week' } },
      series: [{ property: 'value' }]
    });
    expect(stringUnit.validation.errors.join('\n')).toMatch(/tickStep\.unit/);
  });
});
