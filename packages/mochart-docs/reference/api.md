...
and `dataProvider` in place of `config` and `data`, which is useful when several
...
- `refresh()` re-reads the current data without a new reference, for hosts
  that mutate their data in place. A default chart rebuilds its provider over
  `data`; a managed chart calls the provider's optional `refresh()` hook,
  then re-reads it.
...
- `getConfigWithoutDefaults` is its inverse: the minimal config, with every
  value that only restates a default removed, which is what a config editor wants to
...
on it (`mochart-chart`, `mochart-title-text`, `mochart-plot`, …), which is useful
...
`null` while a host is still loading, and a config appearing or going away is
