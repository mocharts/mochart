...
3. **Scales & axes** — multiple axes, threshold line, rotated ticks,
   truncated text (reused) + date/time axis (new local).
4. **Animation** — player (new special), rotation (special).
5. **Interaction** — callbacks (new special), focus styles (new local),
   tooltip & crosshair (new local).
6. **Styling & theming** — gradients, color property, stacked labels,
   christmas tree bars (reused) + currentColor chrome (new local).
7. **Data handling** — missing values (reused), chart states (new special).
...
- [x] Wall mode
- [x] Full curation (36 demos across 8 sections)
